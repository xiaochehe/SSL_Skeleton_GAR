...
def pre_normalization(data, zaxis=[5, 11], xaxis=[5, 6]):
    ...
    for i_s, skeleton in enumerate(tqdm(s)):
        ...
        for dim_index in range(2):
            _range = np.max(skeleton[:,:,:,dim_index]) - np.min(skeleton[:,:,:,dim_index])
            skeleton[:,:,:,dim_index] = (skeleton[:,:,:,dim_index] - np.min(skeleton[:,:,:,dim_index]))/_range 
        s[i_s] = skeleton
