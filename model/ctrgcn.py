...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, sage=False, kernel_size=5, dilations=[1,2]):
        ...
        if self.sage:
            ...
            self.sagetcn1 = MultiScale_TemporalConv(out_channels, out_channels, kernel_size=kernel_size, stride=1, dilations=dilations,
                                            residual=False)
            # Shift_tcn(out_channels, out_channels, stride=1)
        if not residual:
            self.residual = lambda x: 0
    ...
    def forward(self, x):
        if self.sage:
            ...
            if M > 1:
                ...
                sagex = gcnx
            y = self.relu(self.sagetcn1(sagex) + self.sageresidual(gcnx))
        else:
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=6,
        ...
        base_channel = 64
        self.l1 = TCN_GCN_unit(in_channels, base_channel, A, residual=False, sage=True, adaptive=adaptive)
        self.l2 = TCN_GCN_unit(base_channel, base_channel, A, adaptive=adaptive)
        self.l3 = TCN_GCN_unit(base_channel, base_channel, A, adaptive=adaptive)
        self.l4 = TCN_GCN_unit(base_channel, base_channel, A, adaptive=adaptive)
        self.l5 = TCN_GCN_unit(base_channel, base_channel*2, A, stride=2, sage=True, adaptive=adaptive)
        self.l6 = TCN_GCN_unit(base_channel*2, base_channel*2, A, adaptive=adaptive)
        self.l7 = TCN_GCN_unit(base_channel*2, base_channel*2, A, adaptive=adaptive)
        self.l8 = TCN_GCN_unit(base_channel*2, base_channel*4, A, stride=2, sage=True, adaptive=adaptive)
        self.l9 = TCN_GCN_unit(base_channel*4, base_channel*4, A, adaptive=adaptive)
        self.l10 = TCN_GCN_unit(base_channel*4, base_channel*4, A, adaptive=adaptive)

        self.fc = nn.Linear(base_channel*4, num_class)
        self.fc_single = nn.Linear(base_channel*4, 7)
    ...
    def forward(self, x):
        ...
        single_x = torch.bmm(single_x_a, single_x_a.transpose(1,2))/single_x_a.size(-1)
