...
class GCN(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, aggregator_type, feat_drop=0., bias=True, activation=None):
        ...
        self.fc_neigh = nn.Linear(self._in_feats, self._out_feats, bias = bias)
    ...
    def forward(self, g, features, PERSON_NUM, JOINT_NUM, edge_weight=None):
        ...
        with g.local_scope():    
            # sampling graph
            if self.training:
                g = dgl.sampling.sample_neighbors(g,[i for i in range(PERSON_NUM*JOINT_NUM)], int((PERSON_NUM-1))*JOINT_NUM*0.975)
            g = g.to('cuda:0')         
            ...
            if self._aggre_type == 'mean':
                ...
                # h_neigh = (g.dstdata['neigh'] + g.dstdata['h'])/(degs.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)+1)
...
class SageGraph(nn.Module):
    ...
    def forward(self, x, PERSON_NUM, JOINT_NUM):
        ...
        src_ids = torch.tensor(strat_joint)
        dst_ids = torch.tensor(end_joint)
        g = dgl.graph((src_ids, dst_ids), num_nodes = total_point)
